    ...
    public partial class Window1 : Window
    {
        ...
        private int Scan(int id, string ug_type, int global_offset, int count)
        {
            ...
            for (var i = 0; i <= ciclov; i++)
            {
                ...
                string sql_post_id = "SELECT id FROM groups_and_users WHERE vk_id = @vk_id";
                ...
                post_exist.Prepare();
                string sql2 = "INSERT IGNORE INTO posts_vk (group_id, vk_id, date, text, likes, reposts, views, repost_text, owner_id, who_add, trash)" +
                    " VALUES(@group_id, @vk_id, @date, @text, @likes, @reposts, @views, @repost_text, @owner_id, @who_add, 0); SELECT LAST_INSERT_ID();";
                MySqlCommand post_insert = new MySqlCommand(sql2, conn);
                post_insert.Prepare();
                string sql3 = "INSERT IGNORE INTO attachments_vk (group_id, post_id, type, vk_id, owner_id, link, date)" +
                            " VALUES(@group_id, @post_id, @type, @vk_id, @owner_id, @link, @date)";
                ...
                if (contents.SelectToken("response") != null)
                {
                    for (var j = 0; j < current_count; j++)
                    {
                        ...
                        if (contents["response"]["items"][j].SelectToken("copy_history") != null)
                        {
                            ...
                            {
                                int copy_history_attachments = 0;
                                copy_history_attachments = contents["response"]["items"][j]["copy_history"][0]["attachments"].Count();
                        ...
                        if (legal_post)
                        {
                            ...
                            foreach (Attachment current_attach in attachments)
                            {
                                current_attach.post_id = post_id;
                            }                                

                            foreach (Attachment current_attach in attachments)
                            {
                                attach_insert.Parameters.Clear();
                                attach_insert.Parameters.AddWithValue("@group_id", current_attach.group_id);
